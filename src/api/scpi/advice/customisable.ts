...
import { identifier, IEndpoint, role } from "../../../models/scpi/common";
...
/**
 * SCPI Module Customisation Advice for SCPI controllers
 */
export class CustomisableController {
     ...
     * @param modules user-defined SCPI modules and interfaces which should be implemented by bridge
     */
        ...
        for (const [moduleRole, moduleIdentifiers] of Object.entries(needed)) {
            for (const moduleIdentifier of moduleIdentifiers) {
                endpoints.push({
                    ...
                    role: moduleRole,
                    url: url.resolve(publicIP, `/scpi/${moduleRole.toLowerCase()}/2.2/${moduleIdentifier}`)
                })
            }
        }
        return endpoints
     ...
     * @param includableID desired SCPI module ID
     * @param includableRole desired SCPI module Role
     * @param modules user-defined SCPI modules to be implemented by bridge
    ...
    /**
     * Extracts needed SCPI modules and interfaces based on IModules object
